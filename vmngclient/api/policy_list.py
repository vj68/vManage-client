...
class PolicyList:
    ...
    MP_DATA_PREFIX_ID = "/dataservice/template/policy/list/dataprefix/{list_id}"
    ...
    def create_data_prefix_list(self, name: str, ip_prefixes: str):
        """
    ...
    def get_data_prefix_lists(self):
        ...
        if not response.ok:
            logger.error("Failed to get data prefix lists. Details: " f"{response.text}")
            return None

        try:
            data = response.json()["data"]
        except Exception as ex:
            logger.error("Caught exception while extracting data for data " f"prefix list. Details: {ex}")
            data = None
    ...
    def delete_data_prefix_list(self, list_id):
        ...
        if not response.ok:
            logger.error(f"Failed to delete data prefix list: {list_id}" f"Details: {response.text}")
            return False

        return True

    def create_site_list(self, name: str, sites: str):
        """
        Creates site list
        """
        payload = {
    ...
    def get_site_lists(self):
        ...
        try:
            ...
            logger.error("Caught exception while extracting data for site " f"list. Details: {ex}")
            data = None
    ...
    def delete_site_list(self, list_id):
        ...
        if not response.ok:
            logger.error(f"Failed to delete site list: {list_id}" f"Details: {response.text}")
            return False

        return True

    def create_vpn_list(self, name: str, vpns: str):
        """
        Creates VPN list
        """
        payload = {
    ...
    def get_vpn_lists(self):
        ...
        try:
            ...
            logger.error("Caught exception while extracting data for VPN " f"list. Details: {ex}")
            data = None
    ...
    def delete_vpn_list(self, list_id):
        ...
        if not response.ok:
            logger.error(f"Failed to delete VPN list: {list_id}" f"Details: {response.text}")
            return False
    ...
    def delete_data_policy(self, definition_id):
        ...
        if not response.ok:
            logger.error(f"Failed to delete data policy: {definition_id}" f"Details: {response.text}")
            return False
    ...
    def get_data_policies(self):
        ...
        try:
            ...
            logger.error("Caught exception while extracting data for data " f"policies. Details: {ex}")
            data = None
    ...
    def delete_vsmart_policy(self, policy_id):
        ...
        if not response.ok:
            logger.error(f"Failed to delete vSmart policy: {policy_id}" f"Details: {response.text}")
            return False
    ...
    def get_vsmart_policies(self):
        ...
        try:
            ...
            logger.error("Caught exception while extracting data for vSmart " f"policies. Details: {ex}")
            data = None
    ...
    def activate_vsmart_policy(self, name):
        ...
        try:
            ...
            logger.info(f"vSmart policy {name} activation started. " f"Task ID: {task_id}")
        except Exception as ex:
            logger.error("Caught exception while extracting vSmart policy " f"activation task ID. Details: {ex}")
            task_id = None
    ...
    def deactivate_vsmart_policy(self, name):
        ...
        try:
            task_id = response.json()["id"]
            logger.info(f"vSmart policy {name} deactivation started. " f"Task ID: {task_id}")
        except Exception as ex:
            logger.error("Caught exception while extracting vSmart policy " f"deactivation task ID. Details: {ex}")
            task_id = None
    ...
    def get_vsmart_config_task_status(self, task_id):
        ...
        if not response.ok:
            logger.error("Failed to get vSmart config task status. " f"Details: {response.text}")
            return None

        try:
            data = response.json()["data"]
        except Exception as ex:
            logger.error("Caught exception while extracting data for vSmart " f"config task status. Details: {ex}")
            data = None
